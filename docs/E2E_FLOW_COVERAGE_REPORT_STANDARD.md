...
- **Module-scoped runs:** you can filter to a single module using Playwright's `--grep @module:<name>` (for example, `npx playwright test --grep @module:auth`). When running a subset, the report still lists other modules as missing because those flows were not executed in that run.
...
5. Add the flow entry to `docs/USER_FLOW_MAP.md` with steps, branches, and coverage status.
6. Run the E2E suite and verify the new flow appears in the report.
...
4. Remove the flow entry from `docs/USER_FLOW_MAP.md`.
5. Run the suite and verify the flow no longer appears and no tests became unmapped.

### Renaming a flow ID

1. Update the key in `flow-definitions.json`.
2. Update **all** `@flow:` tags in spec files to match the new ID.
3. Update the heading and references in `docs/USER_FLOW_MAP.md`.
4. This must be done atomically — a mismatch will cause the old ID to appear as `missing` and the new ID to be auto-detected under the `unknown` module.
...
Not every quality concern warrants a flow definition. Cross-cutting attributes should be validated with purpose-built tools, not with standalone E2E modules.

| Concern | Why it is NOT a flow | Recommended tool |
|---|---|---|
| **Responsive / viewport layout** | Design quality attribute — no user journey involved | Playwright multi-project viewports (see architecture standard §3.7.4.8) |
| **Performance budgets** | Non-functional metric, not a user interaction | Lighthouse CI |
| **Accessibility compliance** | Structural quality attribute | axe-core / Playwright accessibility snapshots |

**Exception:** If a viewport change triggers **different user behavior** (e.g., a mobile hamburger menu replaces the desktop navbar), that specific interaction **is** a flow and belongs in the relevant functional module (e.g., `e2e/app/sidebar-navigation.spec.ts`).

Do not define flow IDs for cross-cutting concerns. They inflate the flow registry without adding signal and dilute the coverage report.
