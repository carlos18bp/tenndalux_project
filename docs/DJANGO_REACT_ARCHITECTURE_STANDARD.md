...
3. [Frontend - React + TypeScript](#3-frontend---react--typescript)
   ...
   - 3.7 [Testing (Frontend)](#37-testing-frontend)
     ...
     - 3.7.4.8 [Responsive / Multi-Viewport Testing](#3748-responsive--multi-viewport-testing)
     ...
     - 3.7.10 [Coverage Goals and Maintenance](#3710-coverage-goals-and-maintenance)
       - 3.7.10.1 [User Flow Map](#37101-user-flow-map-user_flow_mapmd)
...
- **Tasks:** Huey task tests (`tests/tasks/*`) with broker mocks.
...
Responsive layout is a **cross-cutting design property**, not a user flow. It must not be organized as a standalone E2E module.

**Rules:**

1. **Do NOT** create a dedicated `e2e/viewport/` or `e2e/responsive/` module. Tests that only assert `body` visibility at different sizes verify no observable behavior.
2. Use Playwright **`projects`** to define additional viewport sizes. All functional tests run automatically at every configured viewport.
3. If a specific component has **viewport-dependent behavior** (e.g., sidebar collapses on mobile, hamburger menu appears), the test belongs in that component's **owning functional module** (e.g., `e2e/app/sidebar-navigation.spec.ts`).
...
export default defineConfig({
  ...
  projects: [
    ...
    {
      name: 'Mobile Chrome',
      use: { ...devices['Pixel 5'] },   // width 393, touch-enabled
    },
    {
      name: 'Tablet',
      use: { ...devices['iPad Mini'] }, // width 768, touch-enabled
    },
  ],
});
```

By default, `npm run e2e` and `npm run e2e:coverage` execute the full suite across **all** viewports. Use per-viewport scripts to run a single viewport:

| Script | Command | Viewport |
|---|---|---|
| `npm run e2e:desktop` | `playwright test --project="Desktop Chrome"` | 1280 × 720 |
| `npm run e2e:mobile` | `playwright test --project="Mobile Chrome"` | 393 × 851 (Pixel 5) |
| `npm run e2e:tablet` | `playwright test --project="Tablet"` | 768 × 1024 (iPad Mini) |
...
##### 3.7.10.1 User Flow Map (`USER_FLOW_MAP.md`)

The **User Flow Map** is a human-readable documentation artifact that lists every end-to-end navigation flow in the application. While `flow-definitions.json` is the machine-readable source of truth for the reporter, the User Flow Map is its human-readable companion — it adds step-by-step descriptions, branch/variation details, route information, and coverage status for each flow.

**Location:** `docs/USER_FLOW_MAP.md`

**Why it matters:**

- Provides a single place where any team member can understand **all user journeys** without reading spec files.
- Serves as the acceptance-level specification for E2E coverage — if a flow is not in the map, it is not tested.
- Makes onboarding faster: new developers see every navigable path grouped by role.
...
- E2E Coverage: <Covered|Partial|None> (<spec path>)
...
## Admin Flows
### <flow-id>: ... (or "No admin-specific flows")

## Legend / Conventions
- Priority: P1 (critical), P2 (important), P3 (nice-to-have), P4 (exploratory).
...
**Grouping strategy:** Flows are grouped by role. Flows accessible to multiple roles (e.g., guest + user) go under **Shared Flows**. Single-role flows go under their respective role section.

**Flow entry fields:**

| Field | Required | Description |
|---|---|---|
| Module | Yes | Must match the `module` field in `flow-definitions.json` |
| Priority | Yes | P1–P4, must match `flow-definitions.json` |
| Route | Yes | Primary entry route for the flow |
| Roles | Yes | Which user roles exercise this flow |
| Description | Yes | One-line explanation |
| E2E Coverage | Yes | `Covered`, `Partial`, or `None` with spec file path |
| Steps | Yes | Numbered list of the happy-path user actions |
| Branches / Variations | Yes | Bullet list of alternate paths, edge cases, error states |

**Maintenance rules:**

| When | Action |
|---|---|
| Adding a flow to `flow-definitions.json` | Add the corresponding entry to the User Flow Map |
| Removing a flow from `flow-definitions.json` | Remove the entry from the User Flow Map |
| Renaming a flow ID | Update the heading and all references in the map |
| E2E coverage changes (new specs, removed specs) | Update the coverage status and spec path |
| New branches discovered during development | Add them to the Branches / Variations list |

> The User Flow Map and `flow-definitions.json` must always be in sync. A flow that exists in one but not the other signals a maintenance gap.
...
### 5.4 Run All Suites (Sequential Default)

The project includes `scripts/run-tests-all-suites.py`, a test runner that executes backend (pytest), frontend unit (Jest), and E2E (Playwright) suites with a unified final report. It defaults to sequential execution and can be switched to parallel mode.
...
python scripts/run-tests-all-suites.py --backend-args="-k test_auth"
python scripts/run-tests-all-suites.py --e2e-args="--grep @flow:auth"
```

| Flag | Description |
|------|-------------|
| `--skip-backend` / `--skip-unit` / `--skip-e2e` | Skip individual suites |
| `--parallel` | Run suites in parallel instead of sequentially |
| `--resume` | Re-run failed/unknown suites using `last-run.json` |
| `--verbose` | Force verbose output (even in parallel mode) |
| `--quiet` | Force quiet output (even in sequential mode) |
| `--backend-markers EXPR` | pytest marker expression (`-m`) |
| `--backend-args` / `--unit-args` / `--e2e-args` | Extra args forwarded to each runner |
| `--unit-workers N` / `--e2e-workers N` | Control concurrency per suite |
| `--report-dir DIR` | Directory for per-suite log files (default: `test-reports`) |

The final report includes per-suite status, duration, coverage summaries (backend + Jest + E2E flow coverage), and tail output for any failures. Log files are written to `test-reports/`, and the last run summary is stored in `test-reports/last-run.json` with per-suite metadata plus a `run_id`. Without `--resume`, logs and resume metadata are reset; with `--resume`, logs append (including a run header with timestamp/suite/command) and only failed/unknown suites are rerun.

> **Note:** This script is intended for full CI runs or local pre-merge validation. For day-to-day development, prefer focused execution as described in §5.1 and §5.2.
...
- [ ] **E2E Flow Coverage setup (§3.7.5–3.7.10):**
  ...
  - [ ] Create `docs/USER_FLOW_MAP.md` — document all user flows grouped by role with steps, branches, and coverage status (see §3.7.10.1)
