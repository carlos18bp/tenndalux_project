...
Deploys, migrations and service restarts remain operator-run only.

Static-export navigation payloads are part of the public site contract: every
route generated by Next must publish its nested `.txt` payload alongside HTML,
otherwise client-side navigation degrades into repeat 404s.
