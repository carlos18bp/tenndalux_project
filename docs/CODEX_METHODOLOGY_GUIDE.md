# Tenndalux Codex Methodology Guide

Tenndalux uses a **Codex-native repo runtime**. Claude Code and Windsurf are maintained as compatibility layers only — they must stay aligned with the Codex-native source of truth, not redefine it.
...
| `plugin` | Optional packaging format for sharing skills *across* repos (not Tenndalux's daily runtime) |
